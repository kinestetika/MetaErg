...
class CMScan(abc.AbstractBaseClass):
    ...
    def __result_files__(self) -> tuple:
        ...
        return self.spawn_file('cmscan'),

    def __run_programs__(self):
        """Should execute the helper programs to complete the analysis"""
        fasta_file = self.make_masked_contig_fasta_file()
        cmscan_file = self.spawn_file('cmscan')
        rfam_database = Path(self.exec.database_dir, "Rfam.cm")
        if self.exec.threads > 1:
            split_fasta_files = self.make_split_fasta_files(fasta_file, target='contig')
            split_cmscan_files = [Path(cmscan_file.parent, f'{cmscan_file.name}.{i}')
            ...
            utils.run_external(f'cmscan --rfam --tblout {cmscan_file} {rfam_database} {fasta_file}')

    def __read_results__(self) -> int:
        """Should parse the result files and return the # of positives"""
        hits = []
        with open(self.spawn_file('cmscan')) as hmm_handle:
        ...
        for hit in hits:
            ...
            f = MetaergSeqFeature(self, contig)
            f.location = FeatureLocation(hit.query_start - 1, hit.query_end, strand=hit.query_strand)
            f.description = "{} {}".format(hit.hit_id, hit.descr)
            f.inference = 'cmscan'
            if hit.hit_id in NON_CODING_RNA_TYPES.keys():
                f.type = NON_CODING_RNA_TYPES[hit.hit_id]
