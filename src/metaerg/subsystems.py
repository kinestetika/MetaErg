...
DATAFRAME_DATATYPES = str, str, str
DATAFRAME_DATATYPES = {k: v for k, v in zip(DATAFRAME_COLUMNS, DATAFRAME_DATATYPES)}
SUBSYSTEM_DATA = pd.DataFrame(columns=DATAFRAME_COLUMNS)
SUBSYSTEM_DATA = SUBSYSTEM_DATA.astype(DATAFRAME_DATATYPES)
...
for line in subsystems_data.subsystem_data().split('\n'):
    ...
    SUBSYSTEM_DATA.set_index('function')
...
def aggregate(feature_data: pd.DataFrame):
    ...
    for r in aggregated_subsystem_data.itertuples():
        print(r)
