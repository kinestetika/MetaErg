...
SubSystemStats = namedtuple('SubSystemStats', ['#genes', '#hits', 'fraction'])
...
class SubSystem:
    ...
    def __str__(self):
        if len(self.targets):
            return f'{self.name} ({len(self.hits)}/{len(self.targets)}, {100*len(self.hits)/len(self.targets):.1f}%) '
        else:
            return f'{self.name} ({len(self.hits)}) '
    ...
    def add_hit(self, feature_id:str, target:str='none'):
        self.hits.setdefault(target, set()).add(feature_id)
